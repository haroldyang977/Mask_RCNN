...
def main(argv=None):
    ...
    with tf.gfile.FastGFile("./mask_rcnn.pb", 'rb') as f:
...
#        os.system('eog %s &'%(os.path.join(IMAGE_DIR,filename)))
        ...
        img_ph = sess.graph.get_tensor_by_name('input_image_1:0')
        print(img_ph)
        img_anchors_ph = sess.graph.get_tensor_by_name('input_anchors_1:0')
        print(img_anchors_ph)
        img_meta_ph = sess.graph.get_tensor_by_name('input_image_meta_1:0')
        print(img_meta_ph)
